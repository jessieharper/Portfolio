        ...
        <ul className="flex items-center w-[100%]">
          {TabsData.map((tab, index) => (
            <li
              ...
              className={`bg-card h-full text-xl py-4 p-6 w-[35%] text-center cursor-pointer border-t-2  border-text rounded-t-xl transition-all ease-in-out border-x-2 ${
                index !== activeTab && "noise border-b-2"
              } ${index === activeTab && "underline"} ${
                tab.title === "Skills" && "border-r-2"
              }`}
              key={tab.title}
            >
              {tab.title}
